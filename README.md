...
| `docs/Speaker_Companion.pdf` | **Presenter kit** — per-slide script (amber “SAY” + grey backup), a facts cheat-sheet, a themed Q&A bank of likely audience questions with prepared answers, and a glossary. Regenerate from `docs/companion.html`. |
| `docs/dcm-deck.html` | **The deck to present from** — 15 slides, arrow-key/scroll nav, real SVG diagrams, amber accent. Also published as an Artifact. |
| `docs/DCM_Presentation.pptx` | PowerPoint backup, redesigned to match the HTML deck. Regenerated by `build_deck.py`. |
| `docs/build_deck.py` | Regenerates the deck. Edit content here, not in PowerPoint, if you want the change to survive. |
